...
# -----------------------------------------------------------------------------
def custom_c_profile(qs, c_center=0.0, c_peak=0.3, q_peak=0.5, width=0.1):
    """
    Returns a non-monotonic C profile with a Gaussian bump.
    qs: array of q values (fractional mass)
    c_center: C fraction at center (q=0)
    c_peak:   C fraction at the peak (intermediate q)
    q_peak:   location of the peak (in q)
    width:    width of the bump
    """
    return c_center + (c_peak - c_center) * np.exp(-0.5 * ((qs - q_peak) / width)**2)

def smooth_step(q, q0, width):
    """Smooth step function for transitions."""
    return 1.0 / (1.0 + np.exp(-(q - q0) / width))

def wd_nature_profile(qs):
    # Parameters: adjust to match the paper's figure
    # O core: q < q1, C bump: q1 < q < q2, He envelope: q > q2
    q1 = 0.3   # O/C transition location
    q2 = 0.8   # C/He transition location
    w1 = 0.02  # width of O/C transition
    w2 = 0.02  # width of C/He transition

    # O fraction: high in center, drops at q1
    X_O = 0.8 * (1 - smooth_step(qs, q1, w1))
    # C fraction: rises at q1, falls at q2
    X_C = 0.6 * (smooth_step(qs, q1, w1) - smooth_step(qs, q2, w2))
    # He fraction: rises at q2
    X_He = 1.0 * smooth_step(qs, q2, w2)
    # Normalize so sum <= 1
    total = X_O + X_C + X_He
    X_O /= total
    X_C /= total
...
def conv_wd_slope_highres(
    ...
    # For C (index 3), use a custom profile
    c_profile = np.zeros_like(qs_ramp)  # or your original C profile

    # --- Nature-style profiles for O, C, He ---
    X_O, X_C, X_He = wd_nature_profile(qs_ramp)

    species_vals = []
    for i in range(10):
        if i == 3:
            species_vals.append(X_C)
        elif i == 5:
            species_vals.append(X_O)
        elif i == 2:
            species_vals.append(X_He)
        else:
            species_vals.append(np.zeros_like(qs_ramp))
...
def build_wd_model_slope_highres(
    ...
    logdir = os.path.join("LOGS_improving_C_trend", label)
